...
- `components/AssistantWidget.tsx` — global AI Business Assistant chat widget (mounted in `app/[locale]/layout.tsx`). Channel picker → chat; every reply comes from `POST /api/chat` (streamed into the bubble); transcript persists in `sessionStorage` (`am:chat:history`) and the last 12 messages are sent as context; `am:open-chat` event still opens it with prefilled text. Lead form appears **only** when the visitor is ready (asks for a human/proposal or the assistant hands off) and posts through `/api/contact` with `scenario: "assistant"` plus the transcript. No invented contacts — only email/contact form/`hello@ai-mark.agency`.
