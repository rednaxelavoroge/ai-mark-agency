...
export function CommercialHub({ locale, t }: { locale: Locale; t: Copy }) {
  ...
  const full = t.commercial.tiers.length >= 6;
  const productTier = full
    ? t.commercial.tiers[0]
    : {
        name: isRu ? "AI-продукты" : "AI Products",
        price: [t.products.items.aime.price, t.products.items.assistant.price, t.products.items.showroom.price].join(" · "),
        body: t.products.lead,
      };
  const serviceTier = full ? t.commercial.tiers[1] : null;
  const departmentCopy = full ? t.commercial.tiers[2] : null;
  const productionTier = full
    ? t.commercial.tiers[3]
    : { name: t.production.title, price: t.commercial.custom, body: t.production.lead };
  const customTiers = full ? t.commercial.tiers.slice(4) : [];
  const tabs = [
    ...
    <div>
      ...
      >
        <article className="rounded-xl border border-line bg-ink-2 p-4 sm:p-5">
          <h3 className="font-display text-lg font-semibold text-paper">{productTier.name}</h3>
          <p className="mt-1 font-display text-xl font-bold text-mark">{productTier.price}</p>
          <p className="mt-2 text-xs text-muted">{productTier.body}</p>
      ...
      >
        <article className="rounded-xl border border-line bg-ink-2 p-4 sm:p-5">
          <h3 className="font-display text-lg font-semibold text-paper">
            {serviceTier?.name ?? (isRu ? "AI-маркетинг" : "AI Marketing")}
          </h3>
          <p className="mt-1 font-display text-xl font-bold text-mark">{serviceTier?.price ?? ""}</p>
          <p className="mt-2 text-xs text-muted">{serviceTier?.body ?? t.commercial.lead}</p>
        </article>
        <article className="rounded-xl border border-line bg-ink-2 p-4 sm:p-5">
          <h3 className="font-display text-lg font-semibold text-paper">
            {departmentCopy?.name ?? (isRu ? "Отдел маркетинга" : "AI Marketing Department")}
          </h3>
          <p className="mt-1 font-display text-xl font-bold text-mark">{departmentCopy?.price ?? ""}</p>
          <p className="mt-2 text-xs text-muted">{departmentCopy?.body ?? t.commercial.skuNote}</p>
      ...
      <div role="tabpanel" hidden={active !== "production"} aria-labelledby="tab-production" className="mt-4">
        <article className="rounded-xl border border-line bg-ink-2 p-4 sm:p-5">
          <h3 className="font-display text-lg font-semibold text-paper">{productionTier.name}</h3>
          <p className="mt-1 font-display text-xl font-bold text-mark">{productionTier.price}</p>
          <p className="mt-2 text-xs text-muted">{productionTier.body}</p>
      ...
      <div role="tabpanel" hidden={active !== "custom"} aria-labelledby="tab-custom" className="mt-4 grid gap-3 md:grid-cols-2">
        {(customTiers.length ? customTiers : t.commercial.tiers).map((tier) => (
