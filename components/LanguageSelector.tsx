  ...
  return (
    <div ref={dropdownRef} className={`relative inline-block ${className}`}>
      ...
      <button
        ...
        className="flex h-8 items-center gap-1.5 rounded-full border border-line bg-paper/5 px-2.5 text-xs font-medium text-paper transition-all hover:border-paper/40 hover:bg-paper/10 sm:h-9 sm:px-3 sm:text-xs"
      >
      ...
      {open ? (
        <div className="absolute right-0 top-full z-50 mt-2 hidden w-72 origin-top-right rounded-2xl border border-line bg-ink-2/95 p-2 shadow-2xl backdrop-blur-xl sm:block animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="p-1.5">
            <div className="relative">
              <input
                ...
                placeholder="Search..."
                className="w-full rounded-lg border border-line bg-ink-3/70 px-2.5 py-1.5 text-xs text-paper placeholder-muted outline-none transition-colors focus:border-mark"
              />
              {search ? (
                <button
                  type="button"
      ...
      {/* Mobile Bottom Sheet Modal (sm:hidden) */}
      {open ? (
        <div className="sm:hidden">
          ...
          <div
            ...
            className="fixed inset-x-0 bottom-0 z-50 max-h-[85vh] flex flex-col rounded-t-3xl border-t border-line bg-ink-2 p-4 pb-8 shadow-2xl animate-in slide-in-from-bottom duration-300"
          >
            {/* Handle pill */}
            <div className="mx-auto mb-3 h-1.5 w-12 rounded-full bg-muted/30" />
            ...
            <div className="flex items-center justify-between pb-3 border-b border-line px-1">
              <div>
                ...
                <p className="text-xs text-muted">
                  Choose your preferred language ({allLocales.length})
                </p>
              </div>
              <button
                type="button"
                onClick={() => setOpen(false)}
                className="grid h-8 w-8 place-items-center rounded-full bg-paper/10 text-paper transition-colors hover:bg-paper/20"
                aria-label="Close"
              >
                ✕
              </button>
            ...
            <div className="py-3">
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-xl border border-line bg-ink-3 px-3 py-2 text-sm text-paper placeholder-muted outline-none focus:border-mark"
                />
                {search ? (
                  <button
                    type="button"
                    onClick={() => setSearch("")}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted hover:text-paper"
                    aria-label="Clear"
                  >
                    ✕
                  </button>
